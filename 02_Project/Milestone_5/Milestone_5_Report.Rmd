...
feature_table <- data.frame(
  ...
  Feature = c(
    "indicator_encoded",
    "precision_scaled",
    "data_quality_score_scaled",
    "dataset_source_encoded",
    "sample_size_tier",
    "type_I"
  ),
  Importance = c(0.342, 0.156, 0.128, 0.095, 0.087, 0.064),
  Interpretation = c(
    "Which health indicator determines value range",
    "Survey precision correlates with values",
    "Higher quality surveys show distinct patterns",
    "Health domain influences scale",
    "Large samples correlate with value ranges",
    "Indicator type affects values"
  )
)

kable(feature_table,
      caption = "Top 6 Predictive Features by Permutation Importance",
      align = c("c", "l", "c", "l"))
```
...
```{r review-cadence}
cadence_table <- data.frame(
  Period = c("Weekly (Month 1)", "Monthly (Ongoing)", "Quarterly", "Annual"),
  Focus = c(
    "Technical metrics, critical bugs",
    "Usage stats, performance trends",
    "Business impact, stakeholder feedback",
    "Program review, retraining decision"
  )
)

kable(cadence_table,
      caption = "Monitoring and Review Cadence",
      align = c("l", "l"))
```
...
The model has been deployed using a three-tier microservices architecture with integrated reporting capabilities:

**Frontend (Next.js) - Port 3000:**

- **Dashboard** (`/project`): Model status, training interface, quick start
- **Predictions** (`/project/predict`): CSV upload, results display, download
- **Reporting** (`/project/reporting`): Interactive health data visualizations
  - Water Sources: Pie charts with provincial filtering
  - Child Mortality: Planned
  - Antenatal Care: Planned
...
2. Next.js validates file and sends to Flask API
3. Flask validates schema and calls ML service
4. ML service preprocesses, predicts, and inverse transforms
5. Flask returns predictions to Next.js
6. Next.js displays results table and chart, enables CSV download

**Reporting Workflow:**

1. User navigates to reporting dashboard
2. Next.js loads CSV data from `02_Project/Data/Flat Data/`
3. Interactive charts render with Recharts library
4. User filters by province and explores trends
5. Data disclaimers inform users about data sources
...
- `feature_importance.json` - Feature rankings from Milestone 4
...
- Pre-processed following Milestone 2 pipeline

**Output Format:**

- `predicted_value_log_scaled` - Scaled prediction (z-score normalized)
- `predicted_value` - Original percentage/rate (inverse-transformed)

**Transformation Chain:**

1. Training: value → log1p(value) → z-score normalize → model input
2. Prediction: model output → unscale (z-score) → expm1 → original percentage
...
- Current metrics: R², RMSE, training date
- Status indicator: Loaded/Not Loaded
- Training samples count

**Actions:**

- "Train Model" button (uses local `train_data.csv`)
- Navigation to Predictions and Reporting
- Quick Start instructions

**Action Cards:**

- Make Predictions: Upload CSV for batch predictions
- Model Insights: Feature summary and training details

### Predictions Page (`/project/predict`)

**Features:**

1. **File Upload:** Drag-and-drop or click to select CSV
2. **Preview Mode:** First 10 predictions with comparison table
3. **Full CSV Download:** All predictions with both scaled and original values
4. **Results Display:**
   - Table: Row, Predicted Value (%), Actual Value (%), Difference, Type, Importance
   - Color-coded differences (green < 5%, yellow 5-10%, red > 10%)
   - Bar chart: Prediction errors with reference lines
5. **Help Information:** Input format guidance, example workflow

### Reporting Dashboard (`/project/reporting`)

**Current Reports:**

**Water Source Distribution (`/project/reporting/water`):**

- **Visualization:** Interactive pie chart with provincial filtering
- **Data Source:** `02_Project/Data/Flat Data/Water_Provincial.csv`
- **Categories:** 11 water source types (improved, piped, public tap, borehole, rainwater, tanker, bottled, unimproved, surface, other, missing)
- **Features:**
  - Province dropdown (All Provinces, Gauteng, KwaZulu-Natal, Western Cape, etc.)
  - Percentage labels on pie slices
  - Detailed breakdown table
  - Key insights panel
- **Data Disclaimer:** Provincial data randomly generated for demonstration

**Planned Reports:**

- Child Mortality Rates: Trends by province and year
- Antenatal Care Coverage: Service access and quality metrics
...
tech_stack <- data.frame(
  Component = c("Frontend", "Backend API", "ML Framework", "Data Processing", "Visualization", "Automation (Planned)"),
  Technology = c("Next.js", "Flask", "scikit-learn", "pandas", "Recharts", "n8n"),
  Version = c("15.1.0", "3.0.0", "1.3.2", "2.1.0", "2.5.0", "1.28")
)
...
2. Creates Python virtual environments (`venv`)
3. Installs Python requirements (`pip install -r requirements.txt`)
4. Starts Next.js development server (Port 3000)
5. Starts Flask Task API (Port 5000)
6. Starts Flask ML API (Port 5001)
...
├── Data/
│   └── Flat Data/
│       ├── Water_Provincial.csv  (Reporting data)
│       └── [other datasets]
└── Milestone_5/
...
└── project/
    ...
    └── reporting/
        ...
        └── water/page.tsx        (Water visualization)
```
...
retention_table <- data.frame(
  Category = c("Active", "Backup", "Archive"),
  Location = c(
    "02_Project/Models/Deployed/active/",
    "02_Project/Models/Deployed/backup/",
    "02_Project/Models/Archive/"
  ),
  Retention = c(
...
kable(retention_table,
      caption = "Model Version Retention Policy",
      align = c("l", "l", "l"))
```
...
- Model file (.pkl)
- Preprocessing specs (scalers, encoders)
- Training notebook/logs
- Evaluation report (Milestone 4)
- Monitoring snapshot
...
```{r raci-matrix}
raci_table <- data.frame(
  Activity = c(
    "Monitoring metric review",
    "Retraining execution",
    "Model promotion decision",
    "Quarterly governance meeting"
  ),
  Responsible = c("Data Scientist", "Data Scientist", "Data Scientist", "BI Manager"),
  Accountable = c("BI Manager", "BI Manager", "BI Manager", "BI Manager"),
  Consulted = c("Engineer", "Engineer, QA", "Engineer, QA", "Data Scientist, Engineer"),
  Informed = c("QA Lead", "Stakeholders", "Stakeholders", "Stakeholders")
)

kable(raci_table,
      caption = "RACI Matrix for Maintenance Activities",
      align = c("l", "l", "l", "l", "l"))
```
...
alert_table <- data.frame(
  Status = c("Green", "Amber", "Red"),
  Condition = c(
    "All metrics within thresholds",
    "Single metric near threshold",
    "Threshold breach or 2+ amber"
  ),
  Action = c(
    "Continue monitoring",
    "Manual review within 3 days",
    "Create maintenance ticket, escalate to BI Manager (24h)"
  )
)

kable(alert_table,
      caption = "Alert Levels and Response Actions",
      align = c("l", "l", "l"))
```
...
- ML prediction interface with CSV upload, preview, and download
- Interactive reporting dashboards (water source distribution with provincial filtering)
- Professional UI with drag-and-drop, charts, and contextual help
...
**Model Type:** Ensemble (Random Forest with 750 trees)

**Target Variable:** Survey indicator values (health metrics as percentages/rates)

**Performance:** R² = 0.997, RMSE = 0.0554, MAE = 0.0382

**Training Data:** 560 DHS survey records (1998-2016), South Africa, 7 health domains

**Features:** 27 engineered features (precision, indicator encoding, sample size tier, data quality scores)
...
- No personal data (aggregated statistics only)
- Fairness monitoring required (subgroup error parity)
- Transparent communication of limitations
- POPIA compliant (no personal identifiers)
...
api_endpoints <- data.frame(
  ...
  Description = c(
    "Service health check (returns model_loaded status)",
    "Model metadata (version, metrics, training date, features)",
    "Train model using local train_data.csv",
    "Full CSV predictions (download all predictions)",
    "Preview predictions (first 10 rows)"
  )
)
...
## Appendix C: Reporting Data Sources

```{r data-sources}
data_sources <- data.frame(
  Report = c("Water Source Distribution", "Child Mortality (Planned)", "Antenatal Care (Planned)"),
  Data_Source = c(
    "02_Project/Data/Flat Data/Water_Provincial.csv",
    "02_Project/Data/Flat Data/Child_Mortality.csv",
    "02_Project/Data/Flat Data/Antenatal_Care.csv"
  ),
  Categories = c(
    "11 water source types × 9 provinces",
    "Mortality rates by province and year",
    "ANC coverage and quality metrics"
  ),
  Status = c("Implemented", "Planned", "Planned")
)

kable(data_sources,
      caption = "Reporting Dashboard Data Sources",
      align = c("l", "l", "l", "c"))
```

**Data Disclaimer:** Provincial data in reporting dashboards are randomly generated for demonstration purposes. Only national-level data is based on official survey sources. Provincial breakdowns should not be used for actual analysis or decision-making.
