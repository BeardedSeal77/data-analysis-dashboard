...
missingness_for_gt <- missingness_2d
for(col in numeric_cols) {
  missingness_for_gt[[col]] <- ifelse(missingness_2d[[col]] == "N/A", NA, as.numeric(missingness_2d[[col]]))
}

# Get the actual range of values for proper scaling
all_values <- unlist(missingness_for_gt[numeric_cols])
all_values <- all_values[!is.na(all_values)]
max_val <- if(length(all_values) > 0) max(all_values) else 1

gt::gt(missingness_for_gt) |>
