...
# Find strong correlations based on magnitude
cor_threshold <- 0.7
strong_cors <- which(abs(cor_matrix) > cor_threshold & cor_matrix != 1, arr.ind = TRUE)

if (nrow(strong_cors) > 0) {
  strong_pairs <- data.frame(
    Indicator1 = rownames(cor_matrix)[strong_cors[,1]],
    Indicator2 = colnames(cor_matrix)[strong_cors[,2]],
    Correlation = cor_matrix[strong_cors],
    Strength = ifelse(cor_matrix[strong_cors] > 0, "Positive", "Negative"),
    Abs_Correlation = abs(cor_matrix[strong_cors])
  ) %>%
    arrange(desc(Abs_Correlation))

  cat("STRONG CORRELATIONS FOUND:\n")
  cat("Total strong correlations (|r| >", cor_threshold, "):", nrow(strong_pairs), "\n")
  cat("Strongest positive correlation:", round(max(strong_pairs$Correlation), 3), "\n")
  if (any(strong_pairs$Correlation < 0)) {
    cat("Strongest negative correlation:", round(min(strong_pairs$Correlation), 3), "\n")
  }

  # Display as formatted table
  kable(strong_pairs,
        caption = paste("Strong Correlations (|r| >", cor_threshold, ")"),
        digits = 3) %>%
    kable_styling(bootstrap_options = c("striped", "hover"))

  # Save strong correlations
  write.csv(strong_pairs, "Data/strong_correlations.csv", row.names = FALSE)
  cat("Strong correlations saved to Data/strong_correlations.csv\n")
...
if (exists("strong_pairs") && nrow(strong_pairs) > 0) {
  cat("- Strong correlations found:", nrow(strong_pairs), "\n")
  cat("- Multicollinearity considerations required for modeling\n")
} else {
  cat("- No strong correlations above threshold\n")
...
# Save results (include strong_pairs if it exists)
if (exists("strong_pairs")) {
  save(feature_variance, cor_matrix, weight_strategy, strong_pairs,
       file = "Data/task_04_results.RData")
} else {
  save(feature_variance, cor_matrix, weight_strategy,
       file = "Data/task_04_results.RData")
}
...
- **Feature Independence**: `r if(exists("strong_pairs") && nrow(strong_pairs) > 0) paste("Found", nrow(strong_pairs), "strong correlations requiring multicollinearity consideration") else "Features show good independence, beneficial for modeling"`
